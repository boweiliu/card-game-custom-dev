...
export const MIGRATIONS: Migration[] = [
    ...
    up: async (db: Database): Promise<void> => {
      return new Promise((resolve, reject) => {
        db.serialize(() => {
          ...
          db.run(`INSERT INTO protocards (id, text_body, created_at, updated_at)
            SELECT 
              id, 
              text_body,
              CASE 
  ...
  },
  {
    ...
    description: 'Fix timestamp format to proper ISO with T separator and milliseconds',
    up: async (db: Database): Promise<void> => {
      return new Promise((resolve, reject) => {
        db.serialize(() => {
          // Create backup table with old data
          ...
          db.run(`INSERT INTO protocards (id, text_body, created_at, updated_at)
            ...
            db.run(`DROP TABLE protocards_backup`, (err) => {
              ...
              console.log('✅ Migration 002_fix_iso_timestamp_format completed');
              resolve();
            });
          });
        });
  ...
  for (const migration of MIGRATIONS) {
    if (!appliedMigrations.includes(migration.id)) {
      ...
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          db.run('INSERT INTO migrations (id) VALUES (?)', [migration.id], (err) => {
            if (err) reject(err);
            else resolve();
          });
        });
